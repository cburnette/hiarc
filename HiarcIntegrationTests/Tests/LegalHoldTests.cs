...
{
    public class LegalHoldTests : HiarcTestBase
    {
        [Fact]
        public async void LegalHoldCRUD()
        {
            var c1 = await _hiarc.CreateLegalHold();
            var fetchedLegalHold = await _hiarc.GetLegalHold(c1.Key);
            Assert.Equal(c1, fetchedLegalHold, new EntityComparer());
